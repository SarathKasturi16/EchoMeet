...
export async function getMyFriends(req, res) {
    try {
        const user = await User.findById(req.user.id)
            .select("friends")
            .populate("friends", "fullName profilePic nativeLanguage gender");
...
export async function getFriendRequests(req, res) {
    ...
    try {
        ...
        const acceptedReqs = await FriendRequest.find({
            sender: req.user.id,
            status: "accepted"
        }).populate("recipient", "fullName profilePic gender")
...
export async function deleteFriend(req, res) {
    try {
        ...
        // Also delete any existing FriendRequest records between them
