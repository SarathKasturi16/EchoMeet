...
export async function onboard(req, res) {
    try {
        ...
        const { fullName, bio, nativeLanguage, location, gender } = req.body;
        if (!fullName || !bio || !nativeLanguage || !location || !gender) {
            return res.status(400).json({
                message: "All fields are required",
                missingFields: [
                    !fullName && "fullName",
        ...
        try {
            ...
            console.log("Error in onboard controller", StreamError.message);
        }
